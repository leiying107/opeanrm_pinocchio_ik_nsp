...
class IKNode(Node):
    def __init__(self) -> None:
        super().__init__("openarm_pinocchio_nsp")
        ...
        # Cold-start seed: the URDF midpoint (home) is a kinematic SINGULARITY
        # (sigma_min == 0). Seed instead from a well-conditioned pose (hands_up,
        # joint4=2 rad -> sigma_min ~0.104) so the first IK doesn't diverge.
        _seed = np.zeros(7)
        _seed[3] = 2.0
        self.current_q = np.clip(_seed, self.model.lower, self.model.upper)
