...
> 更新：2026-08-27。本文档是全部工作的总入口：系统全貌、文档地图、
...
| **TRAJECTORY_TEST.md**（204 行）| 轨迹录制回放的仿真验证体系（R1-R5 回归 + TL1-TL6 压测 157 场景）与测试方法论 |
...
### 3.7 轨迹录制回放 + 运动中阻抗
- 拖动录制（50Hz 关节角采样 + 滑动滤波 + 静止段裁剪）→ JSON 持久化 →
  倍率回放（0.2-1.0x，五次缓入缓出）；回放中受外力则暂停-回位-继续；
  任意点匀速回轨迹起点
- 验证体系：R1-R5 回归 + TL1-TL6 压测 157 场景（TRAJECTORY_TEST.md）
...
### 5.9 面板曲线冻结
- **现象**：加入轨迹回放功能后，面板启动 ~1.2s 曲线停更
  （250Hz 线程静默停摆，无错误日志；定位方法：ctrl CSV 行数 vs 时间）
- **解决**：dashboard 定位回 v8 定点阻抗版本，实机曲线恢复正常
- **查看**：USAGE.md §5.1（当前启动流程）
